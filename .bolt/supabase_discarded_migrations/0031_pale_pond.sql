/*
  # Add user_id to tenants table and update policies

  1. Changes
    - Add user_id column to tenants table
    - Create index for user_id column
    - Update RLS policies to allow tenant self-management

  2. Security
    - Allow tenants to manage their own data
...
DROP POLICY IF EXISTS "Allow tenant self-management" ON tenants;
DROP POLICY IF EXISTS "Allow tenant creation" ON tenants;
DROP POLICY IF EXISTS "Allow tenant deletion" ON tenants;
DROP POLICY IF EXISTS "Enable tenant self-management" ON tenants;
