-- Phase 2: Database Schema Updates and Data Consolidation
-- Migration: 20250115000002_phase2_consolidate_rental_data.sql
...
-- Migrate tenants with rental data but no corresponding lease
INSERT INTO leases (
    tenant_id,
    unit_id,
    rent_amount,
...
SELECT 
    t.id as tenant_id,
    NULL as unit_id, -- Will be updated later if unit is found
...
-- Migrate units with rental data but no corresponding lease
INSERT INTO leases (
    tenant_id,
    unit_id,
    rent_amount,
...
SELECT 
    NULL as tenant_id, -- Will be updated later if tenant is found
    u.id as unit_id,
    ...
    COALESCE(u.rent_frequency, 'monthly') as rental_frequency,
    'active' as status,
    NOW() as created_at,
    NOW() as updated_at
FROM units u
