...
export default function RentalDetailsForm({ value, onChange, errors, disabled }: RentalDetailsFormProps) {
  const handleChange = (field: keyof RentalDetails, fieldValue: any) => {
  ...
  return (
    <div className="space-y-4">
      ...
      {value.rental_type === 'rent' && (
        <>
          <div className="space-y-2">
            <label className="block text-sm font-medium text-gray-700">
              Rental Frequency <span className="text-red-500">*</span>
            </label>
            <select
      ...
      {value.rental_type === 'lease' && (
        <>
          <InputField
            label="Lease Amount"
            type="number"
            value={value.lease_amount || ''}
            onChange={(e) => handleChange('lease_amount', e.target.value)}
